...
function handlePlaybackRateChange(e) {
  ...
  const offsetY = e.touches ? e.touches[0].pageY - rect.top : e.offsetY;

  const percent = (offsetY / speed.offsetHeight) * 100;
  const max = 4;
  const min = 0.5;

  let playbackRate = (percent / 100) * (max - min) + min;

  playbackRate = Math.min(Math.max(playbackRate, min), max);

  bar.innerText = playbackRate.toFixed(2) + 'x';
  bar.style.height = `${percent}%`;

  const updatedPercent = ((playbackRate - min) / (max - min)) * 100;

  bar.style.height = `${updatedPercent}%`;
  video.playbackRate = playbackRate;
}

speed.addEventListener('pointermove', handlePlaybackRateChange);

speed.addEventListener('touchmove', handlePlaybackRateChange);
